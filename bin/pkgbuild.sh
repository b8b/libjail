...
: ${PKGBUILD_CACHE_BASE:="$ROOTDIR"/cache}
: ${PKGBUILD_INSTALL_JRE:=false}
: ${PKGBUILD_SITE:=pkg.FreeBSD.org}
: ${PKGBUILD_KEYS:=/usr/share/keys/pkg}
...
cache_root="$PKGBUILD_CACHE_BASE"/FreeBSD-"$v_maj"-"$arch"
...
fetch_pkg()
{
  ...
  $fetch_cmd "$cache_root"/var/cache/pkg/pkg.pkg.sig \
    https://"$PKGBUILD_SITE"/FreeBSD:"$v_maj":"$arch"/latest/Latest/pkg.pkg.sig
  ...
  if grep -qr "${chksum##* }" "$PKGBUILD_KEYS"/revoked 2>/dev/null; then
    echo "signature fingerprint found in $PKGBUILD_KEYS/revoked" >& 2
    exit 1
  fi
  if ! grep -qr "${chksum##* }" "$PKGBUILD_KEYS"/trusted; then
    echo "signature fingerprint not found in $PKGBUILD_KEYS/trusted" >& 2
    exit 1
  fi

  $fetch_cmd "$cache_root"/var/cache/pkg/pkg.pkg \
    https://"$PKGBUILD_SITE"/FreeBSD:"$v_maj":"$arch"/latest/Latest/pkg.pkg
...
run_pkg()
{
  ...
  INSTALL_AS_USER=yes \
  PKG_DBDIR="$cache_root"/pkgboot/var/db/pkg \
  PKG_CACHEDIR="$cache_root"/var/cache/pkg \
    "$pkg_bin" \
    -C "$cache_root"/pkgboot/etc/pkg.conf \
    -R "$cache_root"/pkgboot/etc/pkg/repos \
    "$@"
}
...
init_pkg()
{
  ...
  if ! [ -e "$cache_root"/pkgboot/var/db/pkg ]; then
    mkdir -p "$cache_root"/pkgboot/var/db/pkg
    run_pkg update
  fi
}
...
install_jre()
{
  ...
  if ! v="$(run_pkg_search_version FreeBSD/openjdk17-jre)";
  then
...
if [ -z "$java_cmd" ]; then
  if [ "$PKGBUILD_INSTALL_JRE" = "true" ] || ! java_cmd="$(command -v java)"; then
    init_pkg
    install_jre
  fi
fi

export INTERCEPT_OCI_RUNTIME_NAME=pkgbuild
export INTERCEPT_RC_JAIL="$BINDIR"/intercept-rcjail.sh
export INTERCEPT_RC_JAIL_JAVA_CMD="$java_cmd"

export PKGBUILD_CACHE_BASE="$PKGBUILD_CACHE_BASE"
