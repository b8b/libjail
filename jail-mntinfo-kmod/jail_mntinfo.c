...
static int jail_mntinfo_modevent(__unused module_t mod, int type, void *unused) {
        switch (type) {
                case MOD_LOAD:
                        break;
                case MOD_UNLOAD:
...
static void
emit_fsid(struct sbuf *sb, struct fsid *fsid)
{
        sbuf_cat(sb, "\",\"fsid\":\"");
        encode_hex(sb, (unsigned) fsid->val[0]);
        encode_hex(sb, (unsigned) fsid->val[1]);
}
...
build_json(struct sbuf *sb, char *jpath, int enforce_statfs, struct thread *td)
{
        ...
        expose_fsid = priv_check_cred(td->td_ucred, PRIV_VFS_UNMOUNT) == 0;
        ...
        TAILQ_FOREACH(mp, &mountlist, mnt_list) {
                if (vfs_suser(mp, td) == 0) {
                        ...
                        if (expose_fsid) {
                                emit_fsid(sb, &mp->mnt_stat.f_fsid);
                        }
                        sbuf_cat(sb, "\"}");
                }
        }
...
sysctl_mntinfojson(SYSCTL_HANDLER_ARGS)
{
        ...
        if (req->td == NULL || req->td->td_ucred == NULL) {
                enforce_statfs = 2;
