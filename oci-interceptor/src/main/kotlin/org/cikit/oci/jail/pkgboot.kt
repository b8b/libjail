...
fun fetchPkg(
    ...
    pkgCacheRoot: Path,
    pkgAbiString: String,
    pkgSite: String = "pkg.FreeBSD.org",
    trustedKeysPath: Path = Path("/usr/share/keys/pkg/trusted")
) {
    ...
    val sigInfo = parseSigData(sigData, loadTrustedKeys(trustedKeysPath))
...
private fun loadTrustedKeys(trustedKeys: Path): Set<String> {
    val result = mutableSetOf<String>()
    for (f in trustedKeys.listDirectoryEntries()) {
        if (f.isReadable()) {
            var function: String = ""
...
private fun parseSigData(
    ...
    require(fingerPrint in trustedKeys) {
